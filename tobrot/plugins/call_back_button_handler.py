...
async def button(bot, update: CallbackQuery):
    ...
    if cb_data.startswith("gUPcancel"):
        ...
        if (int(update.from_user.id) == int(from_usr)) or g:
            ...
            await bot.answer_callback_query(
                ...
                text="who are you? An Human Or Alien🤪🤔🤔🤔 LOL",
                show_alert=True,
                cache_time=0,
            )
        return
    ...
    if "|" in cb_data:
        await bot.answer_callback_query(
            update.id, text="trying to download...(Working Hard)", show_alert=False
        )
    ...
    if cb_data.startswith("cancel"):
        if (update.from_user.id == update.message.reply_to_message.from_user.id) or g:
            await bot.answer_callback_query(
                update.id, text="trying to cancel...(I M Sad U cancelled my Work)", show_alert=False
            )
            if len(cb_data) > 1:
            ...
            await bot.answer_callback_query(
                callback_query_id=update.id,
                text="who are you? Human or Alien 🤪🤔🤔🤔 LOL",
                show_alert=True,
                cache_time=0,
            )
    ...
    if cb_data.startswith("help_msg"):
        if "only" in cb_data:
            buttons = [[InlineKeyboardButton("✖️Close but Why", callback_data="close_help")]]
        else:
            buttons = [
                [
                    InlineKeyboardButton("« Back", callback_data="original_help"),
                    InlineKeyboardButton("✖️Close but Why", callback_data="close_help"),
                ]
            ]
            ...
            if len(g_del_list) != 0:
                for f in g_del_list:
                    if os.path.isfile(f):
                        ...
                        shutil.rmtree(f)
                await update.message.edit_text(f"Deleted {len(g_del_list)} objects 😬 Hm Ur Just Deleting My Homework i dont for u")
            else:
                await update.message.edit_text("Nothing to clear 🙄 Why Ur not Leehing using me")
        else:
            await update.message.edit_text("You are not allowed to do that 🤭 Hehe Make Ur Own Bot For Repo dm @movieskacona")

    elif cb_data == "fuckoff":
        await bot.answer_callback_query(
            update.id, text="trying to cancel...", show_alert=False
        )
        await update.message.edit_text("Okay! fine 🤬 Bro I m Sad U Cancelled my Work")
