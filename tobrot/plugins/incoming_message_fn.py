...
async def incoming_message_f(client, message):
    ...
    credit = await message.reply_text(
        f"🧲 Leeching for you <a href='tg://user?id={g_id}'>🤕(Working Hard LOL)</a>", parse_mode="html"
    )
    # get link from the incoming message
    i_m_sefg = await message.reply_text("processing...(Thx For Using LOL)", quote=True)
    ...
    if rep_mess:
        ...
        if not rep_mess.media or str(file_name).lower().endswith(".torrent"):
            ...
            if user_command == LEECH_COMMAND.lower():
                await i_m_sefg.edit(
                    "No downloading source provided 🙄(Atleast Provide a File I m a Bot not an Human)",
        ...
        await i_m_sefg.edit(
            "No downloading source provided 🙄 (Atleast Provide a cool Link)",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
...
async def rename_tg_file(client, message):
    ...
    if len(message.command) > 1:
        ...
        await message.reply_text(
            "<b> Oops 😬</b>\n\nProvide Name to rename Or I can Just Rename it as Rename LOL..\n\n➩<b>Example</b>: <code> /rename Avengers "
            "Endgame</code>",
            quote=True,
        )
