    ...
    if len(message.command) == 1:
        ...
        if message.reply_to_message is not None:
            ...
            if the_real_download_location:
                ...
                await mess_age.edit_text("😔 Download Cancelled or some error happened Report if any error : @movieskacona")
                return
        ...
        if not message.reply_to_message:
            await message.reply("😔 No downloading source provided 🙄 Bruh Try To Understand Imma an bot not an human😎", quote=True)
            return
        if len(message.command) > 1:
...
async def download_tg(client, message):
    ...
    if message.reply_to_message is not None:
        ...
        if the_real_download_location:
            ...
            await mess_age.edit_text("😔 Download Cancelled or some error happened report of an error occured : @movieskacona")
            return
