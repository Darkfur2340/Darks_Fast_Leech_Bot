    ...
    if os.path.isdir(local_file_name):
        ...
        if os.path.getsize(local_file_name) > TG_MAX_FILE_SIZE:
            ...
            i_m_s_g = await message.reply_text(
                ...
                f"Detected File Size: {d_f_s} 😡Don't Worry I M an Intelligent Bot\n"
...
async def upload_to_gdrive(
    ...
    if os.path.isfile(file_upload):
        ...
        button = []
        button.append(
            pyrogram.InlineKeyboardButton(text="☁Your Files Gdrive", url=f"{gauti}")
        )
        if INDEX_LINK:
            ...
            button.append(
                pyrogram.InlineKeyboardButton(text="⚡Your Index Link", url=f"{tam_link}")
            )
